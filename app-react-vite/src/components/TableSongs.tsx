...
type TableSongsProps={
    songs:Song[]
}
export default function TableSongs({songs}:TableSongsProps) {
  return (
   <>
            ...
            <div className="inline-block min-w-full py-2 align-middle sm:px-6 lg:px-8 bg-white p-5 ">
              <table className="min-w-full divide-y divide-gray-300 ">
                ...
                <tbody className="divide-y divide-gray-200">
                  {songs.map((song) => (
                    <tr key={song.id}>
                      <td className="whitespace-nowrap py-4 pl-4 pr-3 text-sm font-medium text-gray-900 sm:pl-0">{song.title}</td>
                     
                      <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-500">{song.duration}</td>
